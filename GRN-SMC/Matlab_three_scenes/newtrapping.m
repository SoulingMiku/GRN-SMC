...
if(flag==0)         %首先不能在安全范围内，然后才能移动
    ...
    diff_x = bigg3(:, 1) - p_fish(1, 1); % 计算每个点的x坐标与p_fish第一个点的x坐标之间的差距
    valid_indices = find(bigg3(:, 2) > p_fish(1, 2) & diff_x >= 0); % 找到y坐标大于p_fish第一个点的y坐标并且x坐标大于等于p_fish第一个点的x坐标的点的索引
