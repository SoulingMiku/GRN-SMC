...
def read_experiment_data(target_path, hunter_path, obstacle_path):
    ...
    # 验证数据完整性（确保有300个时间步）
    assert len(target_df) == 300, "目标数据应包含300个时间步"
    assert len(hunter_df) == 300, "围捕机器人数据应包含300个时间步"
    assert len(obstacle_df) == 300, "障碍物数据应包含300个时间步"
...
def main():
    # 文件路径（根据实际文件位置修改）
    target_path = "1target.txt"
    hunter_path = "1hunter.txt"
    obstacle_path = "1obstacle.txt"
    ...
    for time_step in range(299):
        ...
        if time_step == 0:
            ...
            robotPosition[2] = hunters[2]
            robotPosition[3] = hunters[0]
        ...
        if time_step + 1 in [1, 100, 200, 299]:
            filename = f"GRN-SMC_ori_{time_step + 1}.pdf"
